...
return {
	pixelEnter = function(self)
		...
		rating2 = love.filesystem.load("sprites/pixel/rating.lua")()
	...
	load = function(self)
		...
		rating.x = girlfriend.x + 200
		rating2.x = girlfriend.x - 200
		if not pixel then
			for i = 1, 3 do
				numbers[i].x = girlfriend.x - 100 + 50 * i + 200
				numbers2[i].x = girlfriend.x - 100 + 50 * i - 200
			end
		else
			for i = 1, 3 do
				numbers[i].x = girlfriend.x - 100 + 58 * i + 200
				numbers2[i].x = girlfriend.x - 100 + 58 * i - 200
			end
		end

		ratingVisibility = {0, 0}
		combo = 0
		combo2 = 0
		...
		if not paused then
			if not doingDialogue then
				...
				for i = 1, 4 do
								...
								for i = 1, #enemyNote do
									if enemyNote[i] and enemyNote[i]:getAnimName() == "on" then
										if (enemyNote[i].y - musicPos <= -280) then
											...
											rating2:animate(ratingAnim, false)
											...
											for i = 1, 5 do
												if ratingTimers2[i] then Timer.cancel(ratingTimers2[i]) end
											end

											ratingVisibility[2] = 1
											rating2.y = girlfriend.y - 50
											for i = 1, 3 do
												numbers2[i].y = girlfriend.y + 50
											end

											ratingTimers2[1] = Timer.tween(2, ratingVisibility, {[2] = 0})
											ratingTimers2[2] = Timer.tween(2, rating2, {y = girlfriend.y - 100}, "out-elastic")
											if combo >= 100 then
												ratingTimers2[3] = Timer.tween(2, numbers2[1], {y = girlfriend.y + love.math.random(-10, 10)}, "out-elastic") -- 100's
											end
											if combo >= 10 then
												ratingTimers2[4] = Timer.tween(2, numbers2[2], {y = girlfriend.y + love.math.random(-10, 10)}, "out-elastic") -- 10's
											end
											ratingTimers2[5] = Timer.tween(2, numbers2[3], {y = girlfriend.y + love.math.random(-10, 10)}, "out-elastic") -- 1's
					...
					if notMissed[noteNum] and #enemyNote > 0 and input:down(curInput2) and ((enemyNote[1].y - musicPos <= -400)) and (enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end") then
						voices:setVolume(1)
	...
	drawRating = function(self, multiplier)
		love.graphics.push()
			...
			graphics.setColor(1, 1, 1, ratingVisibility[2])
			if not pixel then
				rating2:draw()
				if combo2 >= 100 then
					numbers2[1]:draw()
		...
		love.graphics.push()
			...
			if not pixel then
				...
				if not settings.botPlay then
					...
					--]]
					love.graphics.printf("Enemy Score: " .. score2 .. "           Boyfriend Score: " .. score, -1950, 400+downscrollOffset, 1100, "center", 0, 3.5, 3.5)
				end
			end
		love.graphics.pop()
